...
def MiAP(t_indices, tag_rankings):
    iAPs = np.zeros(len(t_indices))
    curr_idx = 0

    im_and_tag_rankings = ((t_indices[i], tag_rankings[i,:,:]) for i in range(tag_rankings.shape[0]))
...
def MiAP_alt(q_indices, tag_rankings):
...
def f1_precision_recall(t_indices, tag_rankings, k=3):
    ...
    for i in range(top_k_relevant.shape[0]):
        relevant_concepts = np.nonzero(relevancy_matrix[t_indices[i], :])[0]
        for j in range(top_k_relevant.shape[1]):
            Np[top_k_relevant[i,j]] += 1
            if top_k_relevant[i,j] in relevant_concepts:
...
def make_loaders_text():
   # init dataset
    ...
    base_loader = pickle.load(open("base_loader.p", "rb"))
...
#    base_db, b_indices = make_db_images(model, train_loader)
#    test_db, t_indices = make_db_images(model, test_loader)
#
...
#            indices[i,j] = b_indices[indices[i,j]] # convert query indices into indices in original dataset
    ...
    t_indices = pickle.load(open("t_indices.p", "rb"))

    #pickle.dump(indices, open("indices.p", "wb"))
    #pickle.dump(t_indices, open("t_indices.p", "wb"))

    return indices, t_indices

def faiss_similarity_alt(model, base_loader, query_loader, k=30):
...
#    
#    pickle.dump(indices, open("indices_alt.p", "wb"))
#    pickle.dump(q_indices, open("q_indices.p", "wb"))
#    
    indices = pickle.load(open("indices_alt.p", "rb"))
    q_indices = pickle.load(open("q_indices.p", "rb"))
...
tag_rankings, q_indices = faiss_similarity_alt(model, base_loader, query_loader)
miap = MiAP_alt(q_indices, tag_rankings)
